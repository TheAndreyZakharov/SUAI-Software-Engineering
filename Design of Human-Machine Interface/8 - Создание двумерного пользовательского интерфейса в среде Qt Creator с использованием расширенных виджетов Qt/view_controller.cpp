...
void View_Controller::addToTableView(car* lastObject, QTableView* tableView, QTableView* tableView2)//cool
{
    ...
    model->setHorizontalHeaderLabels({"Название","Комплектация","Номер","Цвет","Год выпуска","Ставка","Наличие"});
    }
    ...
    carRentalCosts[NumberItem->text()] = cost; // Сохраняем стоимость, используя номер автомобиля в качестве ключа
...
// Получаем номер
    ...
    if (carRentalCosts.contains(carNumber)) {
    double totalCost = carRentalCosts[carNumber]; // Получаем сохраненную стоимость
    QStandardItem* finalCost = new QStandardItem(QString::number(totalCost)); // Создаем элемент для отображения стоимости
...
void View_Controller::setRefundData(QTableView* refundtable,QTableView* booktable,rented& rentinfo,QLineEdit* refundDate,cars& carsinfo){//лайн эдит с датой, список выдач, тблица
